...
name = "${env.WORKER_NAME || "ai-agent-server"}"
...
RSS_TITLE = "${env.RSS_TITLE || "AI Agent Development"}"
...
[[d1_databases]]
binding = "DB"
database_name = "${env.DB_NAME || "ai-agent"}"
database_id = "local"

[[queues.producers]]
binding = "TASK_QUEUE"
queue = "${env.TASK_QUEUE_NAME || env.AI_SUMMARY_QUEUE_NAME || `${env.WORKER_NAME || "ai-agent-server"}-tasks`}"
