...
本文档将指导您从零开始部署 AI Agent 博客系统到 Cloudflare。
...
1. 访问 [AI Agent GitHub 仓库](https://github.com/openRin/AI Agent)
2. 点击右上角的 **"Fork"** 按钮
...
AI Agent 目前支持账号密码登录与 Github OAuth 登录，可二选一或同时配置
...
- **前端页面**: `https://ai-agent-client.pages.dev`（或您在 `PAGES_NAME` 中设置的名称）
- **后端 API**: `https://ai-agent-server.your-account-id.workers.dev`
...
🎉 恭喜！您的 AI Agent 博客已经成功部署并发布了第一篇文章！
...
AI Agent 支持两种存储方式：**Cloudflare R2**（推荐）或 **S3 兼容存储**。
...
如果您希望 AI Agent 将评论通知和友链申请通知发送到机器人、聊天工具或自动化平台，请继续阅读 [Webhook 指南](./webhook)。
